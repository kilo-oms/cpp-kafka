...
std::string MessageFactory::create_snapshot_json(const OrderBookSnapshot& snapshot,
                                                 uint32_t depth) const {
    nlohmann::json j;

    // Add common fields
    add_common_fields(j, snapshot.symbol, snapshot.sequence, snapshot.timestamp_us);
...
std::string MessageFactory::create_cdc_json(const CDCEvent& event) const {
    ...
    // Add common fields
    add_common_fields(j, event.symbol, event.sequence, event.timestamp_us);
...
KafkaMessage MessageRouter::route_snapshot(const std::string& symbol, uint32_t depth,
    ...
    if (config_.use_depth_in_topic) {
        ...
        topic = config_.snapshot_topic_prefix.substr(0, config_.snapshot_topic_prefix.length() - 1); // Remove trailing _
    }

    uint32_t partition = config_.use_symbol_partitioning ? calculate_partition(symbol) : -1;
