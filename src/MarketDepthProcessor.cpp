...
void MarketDepthProcessor::processing_loop() {
    ...
    while (!should_stop_) {
        ...
        auto processing_time = get_timestamp_us() - start_time;
...
bool MarketDepthProcessor::process_message(rd_kafka_message_t* msg) {
    ...
    try {
        ...
        // Get envelope
        const auto* envelope = ::md::GetEnvelope(data);
        if (!envelope) {
        ...
        // Check message type
        if (envelope->msg_type() != ::md::BookMsg_OrderBookSnapshot) {
...
void MarketDepthProcessor::publish_snapshots(const OrderBookSnapshot& snapshot) {
    try {
...
void MarketDepthProcessor::print_statistics() const {
    ...
    auto interval_s = std::chrono::duration_cast<std::chrono::seconds>(
        now - metrics_.last_stats_time).count();
    ...
    double avg_processing_time_us = processed > 0 ?
        static_cast<double>(total_processing_time) / processed : 0.0;

    double msg_rate = total_runtime_s > 0 ?
        static_cast<double>(consumed) / total_runtime_s : 0.0;
